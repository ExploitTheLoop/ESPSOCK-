	...
	while (isBeta || (receive((void*)&request) > 0)) {
        ...
        for (int i = 0; i < ulevel.size; i++) {
			...
			if (strstr(name, "BP_PlayerPawn")) {//Player
				...
				if (getI(pBase + 0x954)) //bool bdead
					continue;
                    
				pvm(pBase + 0x918, healthbuff, sizeof(healthbuff)); //health 
				...
				pvm(getA(pBase + 0x140) + 0x150, &exyz, sizeof(exyz));
...
PlayerBone getPlayerBone(uintptr_t pBase, struct D3DMatrix viewMatrix)
{
    ...
    uintptr_t boneAddr = getA(pBase + 0x310);
...
PlayerWeapon getPlayerWeapon(uintptr_t base) {
    ...
    if (isValid32(addr[0]) && getI(addr[0] + 0xdc) == 2) {
        p.isWeapon = true;
        p.id = getI(getA(addr[0] + 0xa60) + 0xcc);
        p.ammo = getI(addr[0] + 0x954);
    }
    else if (isValid32(addr[1]) && getI(addr[1] + 0xdc) == 2) {
        p.isWeapon = true;
        p.id = getI(getA(addr[1] + 0xa60) + 0xcc);
        p.ammo = getI(addr[1] + 0x954);
    }
    else if (isValid32(addr[2]) && getI(addr[2] + 0xdc) == 2) {
        p.isWeapon = true;
        p.id = getI(getA(addr[2] + 0xa60) + 0xcc);
        p.ammo = getI(addr[2] + 0x954);
    }
